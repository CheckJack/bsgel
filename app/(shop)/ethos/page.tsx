...
export default function EthosPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const hasInteracted = sessionStorage.getItem("ethos-modal-interacted") === "true";
      ...
      observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            ...
            const hasInteractedNow = sessionStorage.getItem("ethos-modal-interacted") === "true";
    ...
    try {
      ...
      if (categoriesRes.ok) {
        ...
        if (ethosCategory) {
          ...
          const res = await fetch(`/api/products?categoryId=${ethosCategory.id}`);
          if (res.ok) {
            const data = await res.json();
            setProducts(Array.isArray(data) ? data : data.products || []);
          }
        } else {
          // Otherwise, search for products with "ethos" in the name
          const res = await fetch(`/api/products?search=ethos`);
          if (res.ok) {
            const data = await res.json();
            setProducts(Array.isArray(data) ? data : data.products || []);
          }
    ...
    <>
      ...
      <NailDiagnosisModal 
        ...
        onClose={() => {
          ...
          sessionStorage.setItem("ethos-modal-interacted", "true");
        }}
        onParticipate={() => {
          // Mark as interacted in sessionStorage so it won't show again this session
          sessionStorage.setItem("ethos-modal-interacted", "true");
