...
export default function FootCarePage() {
  ...
  const slides = [
    {
      ...
      title: "Foot Care",
      description: "Discover our premium Foot Care collection",
    },
  ];
  ...
  const fetchFootCareProducts = async () => {
    setIsLoading(true);
    try {
      // Search for products with "foot care" or "foot" in the name
      const res = await fetch(`/api/products?search=foot care`);
      if (res.ok) {
