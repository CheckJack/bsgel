...
export default function HandCarePage() {
  ...
  const slides = [
    {
      ...
      title: "Hand Care",
      description: "Discover our premium Hand Care collection",
    },
  ];
  ...
  const fetchHandCareProducts = async () => {
    setIsLoading(true);
    try {
      // Search for products with "hand care" or "hand" in the name
      const res = await fetch(`/api/products?search=hand care`);
      if (res.ok) {
