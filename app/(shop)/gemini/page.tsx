...
export default function GeminiPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const hasInteracted = sessionStorage.getItem("gemini-modal-interacted") === "true";
      ...
      observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            ...
            const hasInteractedNow = sessionStorage.getItem("gemini-modal-interacted") === "true";
    ...
    try {
      ...
      if (categoriesRes.ok) {
        ...
        if (geminiCategory) {
          ...
          const res = await fetch(`/api/products?categoryId=${geminiCategory.id}`);
          if (res.ok) {
            const data = await res.json();
            setProducts(Array.isArray(data) ? data : data.products || []);
          }
        } else {
          // Otherwise, search for products with "gemini" in the name
          const res = await fetch(`/api/products?search=gemini`);
          if (res.ok) {
            const data = await res.json();
            setProducts(Array.isArray(data) ? data : data.products || []);
          }
    ...
    <>
      ...
      <NailDiagnosisModal 
        ...
        onClose={() => {
          ...
          sessionStorage.setItem("gemini-modal-interacted", "true");
        }}
        onParticipate={() => {
          // Mark as interacted in sessionStorage so it won't show again this session
          sessionStorage.setItem("gemini-modal-interacted", "true");
