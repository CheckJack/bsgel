...
export default function ColourGelsPage() {
  ...
  const slides = [
    {
      ...
      title: "Colour Gels",
      description: "Discover our vibrant Colour Gels collection",
    },
  ];
  ...
  const fetchColourGelProducts = async () => {
    setIsLoading(true);
    try {
      // Search for products with "colour gel" or "color gel" in the name
      const res = await fetch(`/api/products?search=colour gel`);
      if (res.ok) {
