...
export default function TreatmentBaseGelsPage() {
  ...
  const slides = [
    {
      ...
      title: "Treatment Base Gels",
      description: "Discover our premium Treatment Base Gels collection",
    },
  ];
  ...
  const fetchTreatmentBaseGelProducts = async () => {
    setIsLoading(true);
    try {
      // Search for products with "treatment base gel" or "base gel" in the name
      const res = await fetch(`/api/products?search=treatment base gel`);
      if (res.ok) {
