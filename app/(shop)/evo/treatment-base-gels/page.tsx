...
export default function TreatmentBaseGelsPage() {
  ...
  const fetchTreatmentBaseGelProducts = async () => {
    setIsLoading(true);
    try {
      // Fetch products assigned to treatment-base-gels showcasing section with pagination
      const res = await fetch(`/api/products?showcasingSection=treatment-base-gels&page=${currentPage}&limit=10`);
      if (res.ok) {
