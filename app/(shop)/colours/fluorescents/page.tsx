...
export default function FluorescentsPage() {
  const { t } = useLanguage();
  ...
  return (
    <>
      ...
      <section className="relative w-full h-[50vh] md:h-[60vh] overflow-hidden">
          ...
          <Image
            src="/fdhfhffhd.png"
            alt={t("productPagesExtended.coloursFluorescents.hero.alt")}
            fill
            ...
            <div className="max-w-2xl">
              <div className="text-white space-y-1 drop-shadow-lg">
                <p className="text-lg md:text-xl lg:text-2xl font-normal leading-tight">
                  {t("productPagesExtended.coloursFluorescents.hero.line1")}
                </p>
                <p className="text-lg md:text-xl lg:text-2xl font-normal leading-tight">
                  {t("productPagesExtended.coloursFluorescents.hero.line2")}
                </p>
                <p className="text-lg md:text-xl lg:text-2xl font-normal leading-tight">
                  {t("productPagesExtended.coloursFluorescents.hero.line3")}
                </p>
                <p className="text-lg md:text-xl lg:text-2xl font-normal leading-tight">
                  {t("productPagesExtended.coloursFluorescents.hero.line4")}
                </p>
                <p className="text-lg md:text-xl lg:text-2xl font-normal leading-tight">
                  {t("productPagesExtended.coloursFluorescents.hero.line5")}
                </p>
              </div>
      ...
      <section {...(shouldShowId && { id: "products" })} ref={productsSectionRef} className="relative w-full min-h-screen bg-gradient-to-b from-gray-50/30 via-white to-white pt-24 pb-16">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-12">
            <div>
              <ShopProductsTitle>{t("nav.shopMenu.fluorescents")}</ShopProductsTitle>
            </div>
      ...
      <ProductReviews showcasingSection="fluorescents" />
        ...
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
          <div className="text-center text-white px-4">
            <h3 className="text-2xl md:text-3xl lg:text-4xl font-medium mb-4">
              {t("productPagesExtended.coloursFluorescents.videoSection.title")}
            </h3>
            <p className="text-lg md:text-xl mb-6 max-w-2xl font-light">
              {t("productPagesExtended.coloursFluorescents.videoSection.description")}
            </p>
            <Link 
              href="#products" 
              onClick={(e) => {
            ...
            >
              <button className="px-5 py-2 md:px-6 md:py-2.5 border-2 border-white text-white font-normal text-sm md:text-base hover:bg-white/10 transition-colors backdrop-blur-sm">
                {t("productPagesExtended.coloursFluorescents.videoSection.cta")}
              </button>
            </Link>
          </div>
        </div>
