  ...
  const filteredPosts = posts.filter((post) => {
    if (searchQuery) {
      return (
        ...
        (post.reviewComments && post.reviewComments.toLowerCase().includes(searchQuery.toLowerCase()))
      );
    }
    return true;
  });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              {filteredPosts.length} {filteredPosts.length === 1 ? 'post' : 'posts'} rejected
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="flex flex-wrap gap-4 items-center flex-1">
              <div className="flex items-center gap-2">
                ...
                <Input
                  placeholder="Search posts or rejection comments..."
                  value={searchQuery}
        ...
        {filteredPosts.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center">
              <XCircle className="h-12 w-12 mx-auto mb-4 text-gray-400 dark:text-gray-500" />
              <p className="text-gray-500 dark:text-gray-400 text-lg font-medium">
                No rejected posts
              </p>
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">
                There are no posts that have been rejected.
              </p>
            </CardContent>
          </Card>
        ) : (
          filteredPosts.map((post) => {
            ...
            return (
              <Card key={post.id} className="border-red-200 dark:border-red-800">
                <CardContent className="p-6">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {post.reviewComments && (
                        <div className="mb-3 p-3 bg-red-50 dark:bg-red-900/20 rounded text-sm text-gray-900 dark:text-gray-100 border border-red-200 dark:border-red-800">
                          <div className="flex items-center gap-2 mb-1">
                            <XCircle className="h-4 w-4 text-red-600 dark:text-red-400" />
                            <strong className="text-red-700 dark:text-red-300">Rejection Reason:</strong>
                          </div>
                          <p className="text-gray-700 dark:text-gray-300">{post.reviewComments}</p>
                        </div>
                      )}
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mb-4">
                        ...
                        {reviewer && (
                          <div className="flex items-center gap-1">
                            ...
                            <span>Reviewed by: {reviewer.name || reviewer.email}</span>
                          </div>
                        )}
                        <div className="flex items-center gap-1">
                          <Clock className="h-3 w-3" />
                          <span>Rejected: {formatDate(post.updatedAt)}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <Button
                          size="sm"
                          onClick={() => handleStatusChange(post.id, "PENDING_REVIEW")}
