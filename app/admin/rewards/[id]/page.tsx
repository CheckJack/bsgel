...
export default function EditRewardPage() {
  ...
  const fetchReward = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`/api/admin/rewards/${params.id}`);
      if (res.ok) {
        const data = await res.json();
        setFormData({
          name: data.name,
          description: data.description || "",
          pointsCost: data.pointsCost.toString(),
  ...
  return (
    <div className="p-6 min-h-screen">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Edit Reward</h1>
      </div>
